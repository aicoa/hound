...
func nmapServices() []string {
	...
	for _, line := range strings.Split(nmapServicesString, "\n") {
		...
		v1 := line[:index]
		v2 := line[index+1:]
		port, err := strconv.Atoi(v1)
		if err != nil {
			// 如果端口号不是整数，处理错误
			continue
		}

		protocol := FixProtocol(v2)

		// 有效地确保切片有足够的元素
		if port >= len(r) {
			extraSize := port - len(r) + 1
			extraUnknowns := make([]string, extraSize)
			for i := range extraUnknowns {
				extraUnknowns[i] = "unknown"
			}
			r = append(r, extraUnknowns...)
		}
